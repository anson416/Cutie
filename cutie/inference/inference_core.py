...
class InferenceCore:
    def __init__(
        ...
        *,
        ...
        if stagger_updates >= self.mem_every:
            ...
            self.stagger_ti = set(
                np.round(
                    np.linspace(1, self.mem_every, stagger_updates)
                ).astype(int)
            )
    ...
    def _segment(
        ...
        if self.flip_aug:
            ...
            pred_prob_with_bg = (
                pred_prob_with_bg[0]
                + torch.flip(pred_prob_with_bg[1], dims=[-1])
            ) / 2
        else:
    ...
    def step(
        ...
        *,
        ...
        """
        # [Memory Optimization] Disable gradient calculation to prevent graph storage.
        # This significantly reduces VRAM usage (no activations stored) and prevents
        # graph memory leaks across frames.
        with torch.no_grad():
            ...
            update_sensory = (
                (self.curr_ti - self.last_mem_ti) in self.stagger_ti
            ) and (not end)
            ...
            # [Memory Optimization] Detach last_mask.
            # Even with torch.no_grad(), explicit detach ensures no history is accidentally
            # retained if this object is inspected outside this context.
            self.last_mask = pred_prob_with_bg[1:].unsqueeze(0).detach()
            if self.flip_aug:
                self.last_mask = torch.cat(
                    [self.last_mask, torch.flip(self.last_mask, dims=[-1])],
                    dim=0,
                )
            ...
            # [Memory Optimization] Delete the high-res feature buffer.
            # This frees the heavy tensors (ms_feat, pix_feat, key) immediately.
            if delete_buffer:
                self.image_feature_store.delete(self.curr_ti)
            ...
            # [Memory Optimization] Return detached tensor.
            # Ensures the user cannot accidentally hold onto the graph via the return value.
