...
class MemoryManager:
    ...
    def _get_mask_by_ids(
        self, mask: torch.Tensor, obj_ids: List[int]
    ) -> torch.Tensor:
    ...
    def _get_visual_values_by_ids(self, obj_ids: List[int]) -> torch.Tensor:
        ...
        value = torch.stack(
            [self.work_mem.value[obj] for obj in obj_ids], dim=1
        )
        if self.use_long_term and obj_ids[0] in self.long_mem.value:
            lt_value = torch.stack(
                [self.long_mem.value[obj] for obj in obj_ids], dim=1
            )
    ...
    def read(
        ...
        """Optimized memory read with reduced tensor operations."""
        h, w = pix_feat.shape[-2:]
        bs = pix_feat.shape[0]

        # Flatten once and reuse
        query_key_flat = query_key.flatten(start_dim=2)
        selection_flat = selection.flatten(start_dim=2)

        all_readout_mem = {}
        buckets = self.work_mem.buckets

        for bucket_id, bucket in buckets.items():
            # Compute affinity once per bucket
            if self.use_long_term and self.long_mem.engaged(bucket_id):
                ...
                similarity = get_similarity(
                    memory_key, shrinkage, query_key_flat, selection_flat
                )
            ...
            chunk_size = (
                self.chunk_size if self.chunk_size >= 1 else len(bucket)
            )

            for i in range(0, len(bucket), chunk_size):
                objects = bucket[i : i + chunk_size]
                num_objs = len(objects)

                # Batch gather operations
                this_sensory = torch.stack(
                    [self.sensory[obj] for obj in objects], dim=1
                )
                this_last_mask = last_mask[
                    :,
                    [
                ...
                # Get visual values efficiently
                values_list = [self.work_mem.value[obj] for obj in objects]
                if self.use_long_term and objects[0] in self.long_mem.value:
                    lt_values = [self.long_mem.value[obj] for obj in objects]
                    this_msk_value = torch.stack(
                        [
                            torch.cat([lt, wk], dim=-1)
                            for lt, wk in zip(lt_values, values_list)
                        ],
                        dim=1,
                    )
                else:
                    this_msk_value = torch.stack(values_list, dim=1)

                # Readout
                visual_readout = self._readout(affinity, this_msk_value).view(
                    bs, num_objs, self.CV, h, w
                )
                ...
                this_obj_mem = (
                    this_obj_mem.unsqueeze(2)
                    if this_obj_mem is not None
                    else None
                )
                ...
                for j, obj in enumerate(objects):
                    all_readout_mem[obj] = readout_memory[:, j]

                if self.save_aux:
                    self.aux = {
                        "sensory": this_sensory,
                        "pixel_readout": pixel_readout,
                        "q_logits": aux_features["logits"]
                        if aux_features
                        else None,
    ...
    def add_memory(
        ...
        *,
        ...
        msk_values = {
            obj: msk_value[:, obj_id] for obj_id, obj in enumerate(objects)
        }
        self.work_mem.add(
            key,
            msk_values,
    ...
    def purge_except(self, obj_keep_idx: List[int]) -> None:
        ...
        self.sensory = {
            k: v for k, v in self.sensory.items() if k in obj_keep_idx
        }
